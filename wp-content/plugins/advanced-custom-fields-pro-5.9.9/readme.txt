...
* Enhancement - New Field Groups admin.
    ...
    * Added new columns: Description, Key, Location, Local JSON.
    * Added popup modal to review Local JSON changes before sync.
    * Added visual representation of where Field Groups will appear.
    * Improved design of multiple elements.
    * Simplified layout.
* Enhancement - Added support for `<innerBlocks />` within ACF Blocks.
* Enhancement - Added ability to duplicate Repeater and Flexible Content values.
* Enhancement - Added field validation for Gutenberg.
* Enhancement - Added ability to bypass confirmation tooltips.
...
* Dev - Major improvements to `ACF_Location` class (documentation available soon).
...
* Enhancement - Added new block type "align_content" setting and support for the AlignMatrix component.
* Enhancement - Optimized block AJAX fetching by combining calls and reducing the number of requests.
* Tweak - Tweaked styling applied to newly duplicated rows and layouts.
* Fix - Fixed "empty block" bug caused by overuse of `do_blocks()`.
* Fix - Fixed bug breaking the InnerBlocks templateLock attribute.
* Fix - Fixed bug breaking blocks with HTML comments.
* Fix - Fixed bug causing unresponsive WYSIWYG fields after moving a block via the up/down arrows.
* Fix - Fixed bug causing broken "row actions" on the trashed Field Groups admin screen.
* Fix - Fixed bug causing render flicker on the Sync Field Groups admin screen.
* Fix - Fixed bug sometimes displaying validation errors when saving a draft.
* Dev - Added backwards compatibility for changed ACF_Assets class methods.
