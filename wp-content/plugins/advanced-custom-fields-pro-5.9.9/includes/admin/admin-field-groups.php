	...
	public function render_admin_table_column_local_status( $field_group ) {
		...
		if( isset( $json[ $field_group['key'] ] ) ) {
			...
			if( isset($this->sync[ $field_group['key'] ]) ) {
				$url = $this->get_admin_url( '&acfsync=' . $field_group['ID'] . '&_wpnonce=' . wp_create_nonce('bulk-posts') );
...
(function($){
	...
	public function admin_footer__sync() {
		...
		?>
<script type="text/html" id="tmpl-acf-sync-tbody">
	<?php 
		...
		foreach( $columns as $column_name => $column_label ) {
			...
			switch ( $column_name ) {
				...
				case 'title':
					...
					//echo '<div class="row-actions"><span class="sync"><a aria-label="' . esc_attr( __('Synchronise field group', 'acf') ) . '" href="#">' . __( 'Sync', 'acf' ) . '</a></span></div>';
...
<script type="text/javascript">
(function($){
	$('#the-list').html( $('#tmpl-acf-sync-tbody').html() );
})(jQuery);
</script>
		<?php
	}
