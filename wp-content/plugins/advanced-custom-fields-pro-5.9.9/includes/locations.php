...
function acf_register_location_type( $class_name ) {
	 ...
	 * @param	string $location_type The location type instance.
	 */
...
function acf_get_location_rule_operators( $rule ) {
	$operators = ACF_Location::get_rule_operators( $rule );
	
	// Get operators from location type since 5.9.
	$location_type = acf_get_location_type( $rule['param'] );
	if( $location_type ) {
		$operators = $location_type->get_rule_operators( $rule );
	}
...
function acf_get_location_rule_values( $rule ) {
	...
	$location_type = acf_get_location_type( $rule['param'] );
	if( $location_type ) {
		$values = $location_type->get_rule_values( $rule );
	}
...
function acf_match_location_rule( $rule, $screen, $field_group ) {
	...
	$location_type = acf_get_location_type( $rule['param'] );
	if( $location_type ) {
		$result = $location_type->match_rule( $rule, $screen, $field_group );
	}
